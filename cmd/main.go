...
func init() {
	...
	utilruntime.Must(tektonv1.AddToScheme(scheme))        // Add Tekton Pipelines scheme
	utilruntime.Must(triggersv1beta1.AddToScheme(scheme)) // Add Tekton Triggers scheme
...
func main() {
	...
	ctrl.SetLogger(zap.New(zap.UseFlagOptions(&config.loggerOpts)))
	...
	// Setup metrics certificate watcher if needed
	if config.metricsCertPath != "" {
...
func setupFlags() *Config {
	config := &Config{
		loggerOpts: zap.Options{Development: true},
	}
	...
	config.loggerOpts.BindFlags(flag.CommandLine)

	return config
}

// printVersionInfo prints version information and exits
...
// setupTLSOptions configures TLS options based on flags
func setupTLSOptions(config *Config) []func(*tls.Config) {
...
func setupMetricsServer(config *Config, tlsOpts []func(*tls.Config)) metricsserver.Options {
