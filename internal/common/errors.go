...
func NewErrorResult(phase, status, message string, err error, requeueAfter time.Duration) *ReconcileResult {
	return &ReconcileResult{
		Success:      false,
		Error:        err,
		RequeueAfter: requeueAfter,
...
func GetRequeueDelay(retryCount int, baseDelay time.Duration) time.Duration {
	...
	delay = delay + time.Duration(float64(jitter)*(2*float64(time.Now().UnixNano()%1000)/1000-1))
